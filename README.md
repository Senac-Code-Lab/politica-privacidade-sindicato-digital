# Política de Privacidade – Sindicato Digital

Este repositório contém a Política de Privacidade oficial do aplicativo **Sindicato Digital – Sindivarejo Mossoró**.  
A página é publicada por meio do **GitHub Pages** e atende às exigências da Google Play Store para disponibilização do aplicativo.

## 📄 Acesso ao documento
A Política de Privacidade pode ser acessada em:

**https://senac-code-lab.github.io/politica-privacidade-sindicato-digital/**  

## 📱 Sobre o aplicativo
O aplicativo Sindicato Digital oferece acesso a documentos, taxas associativas, benefícios, convênios, informações institucionais e demais serviços fornecidos pelo Sindivarejo Mossoró.

## 📬 Contato
Dúvidas ou solicitações relacionadas à privacidade podem ser enviadas para:
**sindivarejomossoro@gmail.com**

